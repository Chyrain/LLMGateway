...
async def get_dashboard_stats(
    ...
    # 今日请求次数
    today_requests = db.query(func.count(OperationLog.id)).filter(
    ...
    current_quota = None
    if current_model:
        current_quota = db.query(QuotaStat).filter(
            QuotaStat.model_id == current_model.id
        ).first()
    ...
    # 最近切换日志
    switch_logs = db.query(OperationLog).filter(
    ...
    return {
        ...
        "data": {
            ...
            "currentModel": {
                ...
                "used_ratio": round(current_quota.used_ratio, 2) if current_quota else 0
            } if current_model else None,
            "switchLogs": [{
                "id": log.id,
                "from_model": log.log_content.get("from_model") if isinstance(log.log_content, dict) else None,
                "to_model": log.log_content.get("to_model") if isinstance(log.log_content, dict) else None,
                "reason": log.log_content.get("reason") if isinstance(log.log_content, dict) else None,
...
async def get_usage_trend(
    ...
    daily_stats = db.query(
        ...
        OperationLog.log_type == 1,
        OperationLog.create_time >= start_date
    ).group_by(
        func.date(OperationLog.create_time)
    ).order_by(
        func.date(OperationLog.create_time)
    ).all()
    ...
    for i in range(days):
        ...
        full_trend.append({
            "date": date_str,
            "requests": trend_dict.get(str(date), 0)
        })
...
async def get_model_ranking(
    ...
    model_stats = db.query(
        OperationLog.model_id,
        func.count(OperationLog.id).label('request_count')
    ).filter(
        OperationLog.log_type == 1
    ).group_by(
        OperationLog.model_id

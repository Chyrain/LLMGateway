...
async def get_log_list(
    ...
    # 分页查询
    logs = (
        query.order_by(desc(OperationLog.create_time))
        .offset((page - 1) * size)
        .limit(size)
        .all()
    )
        ...
        "data": [
            {
                ...
                "response_time": log.response_time.strftime("%Y-%m-%d %H:%M:%S")
...
async def clear_logs(log_type: Optional[int] = None, db: Session = Depends(get_db)):
    """
    清空日志
    """
    query = db.query(OperationLog)
    ...
    return {"code": 200, "msg": f"已清空 {deleted_count} 条日志"}
...
async def export_logs(
    ...
    for log in logs:
        ...
        csv_lines.append(
            f"{log.id},{log_type_name},{log.model_id or ''},{status_name},{create_time},{content}"
        )
